function [mzroi,roicell,nrows] = nn_RunPartialareaROI
...
for si=1:noFiles
    ...
    prenum=0;
    premzerror=[];
    for nono=1:nobanfeng
        point=rangeg(nono);
        [kk,jj]=find(Indexesii==point);
        try
    ...
    elseif sum(premzerror)==0
        ...
        premzerror= rmoutliers(premzerror);% 剔除离群值
        nmzerror(si)=mean(premzerror);     % 算平均半峰宽
    end
...
if nmzroi>1
    ...
    while check_error==1
        ...
        if isempty(valor)
            ...
            if strcmp(weighted,'default: mean')
                roicell(valor,4) = num2cell(cellfun(@(x) sum(x)/length(x), roicell(valor,1)));
            elseif strcmp(weighted,'median')
                roicell(valor,4) = num2cell(cellfun(@(x) median(x), roicell(valor,1)));
            elseif strcmp(weighted,'weighted')
                roicell(valor,4)=num2cell(cellfun(@(x,y) sum(x.*y)/sum(y),roicell(valor,1),roicell(valor,2)));
            elseif strcmp(weighted,'max')
                roicell(valor,4)=num2cell(cellfun(@(x,y) mean(x(find(y==max(y)))),roicell(valor, 1),roicell(valor,2)));
            end
            
            mzroi(valor)=cell2mat(roicell(valor,4)');
            roicell(valor+1,:)=[];
            mzroi(valor+1)=[];
